from water_flow import water_column_height, pressure_gain_from_water_height, pressure_loss_from_pipe, pressure_loss_from_fittings, reynolds_number, pressure_loss_from_pipe_reduction, kilopascals_to_psi
...
# pytest module is imported to use its main function to run the tests.
def test_kilopascals_to_psi():
  assert kilopascals_to_psi(0.0) == approx(0.000, abs=0.001)
  assert kilopascals_to_psi(10.0) == approx(1.450, abs=0.001)
  assert kilopascals_to_psi(50.0) == approx(7.252, abs=0.001)
  assert kilopascals_to_psi(100.0) == approx(14.504, abs=0.001)
  assert kilopascals_to_psi(200.0) == approx(29.008, abs=0.001)
